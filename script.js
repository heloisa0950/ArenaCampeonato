...
async function carregarTimes(){
    try{
        let response = await fetch("https://api.npoint.io/38382e5d450e00a26e8d") 
        let timesDapi = await response.json();
        console.log("Dados recebidos:",timesDapi);
...
function ativarMaquinaCliques(){
    ...
    todosOsJogos.forEach(function(botao){
        botao.addEventListener("click", function(event){
           ...
           console.log("O juiz já apitouo fim desse confronto!");
        }
